...
function bytesToBase64(bytes){let binary='';const step=0x8000;for(let i=0;i<bytes.length;i+=step)binary+=String.fromCharCode.apply(null,bytes.subarray(i,Math.min(i+step,bytes.length)));return btoa(binary)}
async function exportBlobToNative(blob,name,nativeBridge){const exportId=nativeBridge.beginFileExport(name,blob.type||'application/octet-stream',blob.size);if(!exportId)throw new Error('Android could not start this export.');try{const chunkSize=256*1024;for(let offset=0;offset<blob.size;offset+=chunkSize){const bytes=new Uint8Array(await blob.slice(offset,Math.min(offset+chunkSize,blob.size)).arrayBuffer());if(!nativeBridge.appendFileExportChunk(exportId,bytesToBase64(bytes)))throw new Error('Android could not write the complete export.')}if(!nativeBridge.finishFileExport(exportId))throw new Error('Android could not finish this export.');return true}catch(error){try{nativeBridge.cancelFileExport(exportId)}catch{}throw error}}
function download(blob,name){const nativeBridge=root.AtrangiNative;if(nativeBridge&&typeof nativeBridge.beginFileExport==='function'&&typeof nativeBridge.appendFileExportChunk==='function'&&typeof nativeBridge.finishFileExport==='function'){const pending=exportBlobToNative(blob,name,nativeBridge);pending.catch(error=>{console.error('Native export failed:',error);root.AtrangiScannerApp?.toast?.(error.message||'Unable to export this file.','error')});return pending}const u=URL.createObjectURL(blob),a=document.createElement('a');a.href=u;a.download=name;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(u),2000);return Promise.resolve(true)}
