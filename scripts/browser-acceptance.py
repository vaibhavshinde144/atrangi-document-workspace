...
def record(name, ok, detail=""):
    item={"name":name,"ok":bool(ok),"detail":str(detail)[:1600]}
    checks.append(item)
...
def wait(d,expr,seconds=18):
    return WebDriverWait(d,seconds).until(lambda x:x.execute_script(f"return Boolean({expr})"))


def active(d,panel):
    return d.execute_script("const p=document.getElementById(arguments[0]);return !!p&&p.classList.contains('active-panel')",panel)
...
def runtime_suite():
    ...
    try:
        ...
        wait(d,"document.body && document.body.dataset.brandingTest==='PASS'",22)
        record("runtime app and branding initialized",True)
        record("browser title v7.1.8",d.title=="Atrangi Document Workspace v7.1.8",d.title)
        ...
        record("visible version labels v7.1.8",all("7.1.8" in labels[k] for k in labels),json.dumps(labels))

        logos=d.execute_script("return [...document.querySelectorAll('.brand-logo img')].map(x=>({src:x.getAttribute('src'),w:x.naturalWidth,h:x.naturalHeight,display:getComputedStyle(x).display}))")
        ...
        click(d,'#mobileNav [data-tab="home"],#desktopNav [data-tab="home"]')
        imp=click(d,"#heroImportBtn");time.sleep(.25)
        record("hero Import route",imp and active(d,'filesPanel'),"filesPanel active")
        ...
        d.find_element(By.ID,"universalImportInput").send_keys(str(f))
        imported=WebDriverWait(d,12).until(lambda x:x.execute_script("return Number(document.getElementById('universalCount')?.textContent||0)>=1||document.querySelectorAll('#universalRepo .universal-file-card').length>=1"))
        record("universal text-file import",imported)

        click(d,'#mobileNav [data-tab="tools"],#desktopNav [data-tab="tools"]')
        s=d.find_element(By.ID,"toolSearch");s.clear();s.send_keys("PDF");time.sleep(.25)
        n=d.execute_script("return [...document.querySelectorAll('#toolGroups .tool-card-button')].filter(b=>!b.hidden&&getComputedStyle(b).display!=='none').length")
        record("tools search/filter",n>0,n)

        click(d,'#mobileNav [data-tab="home"],#desktopNav [data-tab="home"]')
        sc=click(d,"#heroScanBtn");time.sleep(.5)
        surface=d.execute_script("return ['scanStartSheet','cameraStage','documentBuilderDialog'].some(id=>{const e=document.getElementById(id);if(!e)return false;if(e.tagName==='DIALOG')return e.open;const s=getComputedStyle(e);return !e.hidden&&s.display!=='none'&&s.visibility!=='hidden'})")
        record("scan entry action",sc and surface,surface)
...
def responsive_suite():
    for w,h,label in [(360,800,'small-mobile'),(390,844,'mobile'),(768,1024,'tablet'),(1440,900,'desktop')]:
        ...
        try:
            ...
            wait(d,"document.getElementById('app')");wait(d,"document.body.dataset.brandingTest==='PASS'",22)
            ...
            no_over=m['scrollWidth']<=m['innerWidth']+2
            head=m['header'] and m['header']['left']>=-2 and m['header']['right']<=m['innerWidth']+2
...
runtime_suite();responsive_suite()
